// ===================== dashboard.js =====================
import { formatDate, getTotals, getCategoryIcon } from './transactions.js';
...
function animateCount(el, from, to, currency, isPercent = false) {
  ...
  const step = (ts) => {
    ...
    if (isPercent) {
      ...
      el.textContent = formatCompactCurrency(current, currency);
    }
...
export function updateSummaryCards(txs, currency) {
  ...
  const step = (ts) => {
    ...
    srEl.textContent =
      (from + (savingsRate - from) * ease).toFixed(1) + '%';

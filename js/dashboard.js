...
// FIX: Removed duplicate `formatCompactCurrency` that diverged from transactions.js.
//      All formatting now goes through the single source-of-truth `formatCurrency`.
...
function animateCount(el, from, to, currency, isPercent = false) {
  ...
  const step = (ts) => {
    ...
    if (isPercent) {
      ...
      // formatCurrency handles INR→USD conversion internally
  ...
  container.innerHTML = items.map(tx => `
    <div class="tx-item" data-id="${tx.id}">
      ...
      <div class="tx-actions">
        <button class="tx-btn edit" data-id="${tx.id}" title="Edit">
          <i class="fa-solid fa-pen-to-square"></i>
        </button>
        <button class="tx-btn del" data-id="${tx.id}" title="Delete">
